...
def check_unique_ids(objects):
  ...
  for obj in objects:
    if obj.id in id_set:
      raise HTTPResponseCustomized(status_code=400, detail="Metrics IDs are not unique, Please don't duplicate your metric")
    id_set.add(obj.id)
  return True
...
def check_metric_weight(objects):
  ...
  if (sum == 100):
    return True
  raise HTTPResponseCustomized(status_code=400, detail="Sum of metric weight is not 100")
  ...
  for obj in objects:
      ...
      if (obj.type == "integer"):
        if (not isinstance(obj.desiredValue, int)):
          raise HTTPResponseCustomized(status_code=400, detail="desierdValue is not correct for integer metric")
      elif (obj.type == "boolean"):
        if (not isinstance(obj.desiredValue, bool)):
          raise HTTPResponseCustomized(status_code=400, detail="desierdValue is not correct for boolean metric")
      elif (obj.type == "string"):
        if (not isinstance(obj.desiredValue, str)):
          raise HTTPResponseCustomized(status_code=400, detail="desierdValue is not correct for string metric")
      elif (obj.type == "float"):
        if (not isinstance(obj.desiredValue, float)):
          raise HTTPResponseCustomized(status_code=400, detail="desierdValue is not correct for float metric")
...
def check_metric_criteria(objects):
  for obj in objects:
    if (obj.criteria not in criteriaStates):
      raise HTTPResponseCustomized(status_code=400, detail="Criteria is not correct")
    
    if (obj.type == "string" or obj.type == "boolean"):
      if (obj.criteria != "equal"):
        raise HTTPResponseCustomized(status_code=400, detail="Criteria is not correct for this metric, you must make it equal")
